    ...
    public class CoreNode
    {
        ...
        public async Task<Block[]> GenerateAsync(int blockCount, bool includeUnbroadcasted = true, bool broadcast = true)
        {
            ...
            using (INetworkPeer peer = this.CreateNetworkPeerClient())
            {
                ...
                var chain = bestBlock == this.runner.Network.GenesisHash ? new ConsensusChainIndexer(this.runner.Network) : this.GetChain(peer);
        ...
        private ConsensusChainIndexer GetChain(INetworkPeer peer, uint256 hashStop = null, CancellationToken cancellationToken = default(CancellationToken))
        {
            var chain = new ConsensusChainIndexer(peer.Network);
