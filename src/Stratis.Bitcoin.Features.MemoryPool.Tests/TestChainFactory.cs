...
namespace Stratis.Bitcoin.Features.MemoryPool.Tests
{
    ...
    internal class TestChainFactory
    {
        ...
        public static async Task<ITestChainContext> CreateAsync(Network network, Script scriptPubKey, string dataDir)
        {
            ...
            {
                Block currentBlock = Block.Load(newBlock.Block.ToBytes(network.Consensus.ConsensusFactory), network, network.Consensus.ConsensusFactory);
            ...
            inMemoryCoinView.SaveChanges(outputs, new List<TxOut[]>(), chain.GetBlock(0).HashBlock, chain.GetBlock(1).HashBlock, chain.GetBlock(0).Height);
