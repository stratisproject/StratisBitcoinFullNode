...
namespace Stratis.Bitcoin.Features.MemoryPool.Tests
{
    public class MempoolPersistenceTest : IDisposable
    {
        ...
        private static MempoolManager CreateTestMempool(NodeSettings settings, out TxMempool txMemPool)
        {
            ...
            ConsensusRules consensusRules = new PowConsensusRules(Network.Main, loggerFactory, dateTimeProvider, chain, new NodeDeployments(Network.Main, chain), consensusSettings, new Checkpoints(), new InMemoryCoinView(new uint256())).Register(new FullNodeBuilderConsensusExtension.PowConsensusRulesRegistration());
