...
namespace Stratis.Bitcoin.Features.MemoryPool.Tests
{
    ...
    {
        public MempoolValidatorTest() : base(NetworkContainer.RegTest)
        {
        }
        ...
        public async Task AcceptToMemoryPool_WithValidP2PKHTxn_IsSuccessfullAsync()
        {
            ...
            var minerSecret = new BitcoinSecret(new Key(), NetworkContainer.RegTest);
            ITestChainContext context = await TestChainFactory.CreateAsync(NetworkContainer.RegTest, minerSecret.PubKey.Hash.ScriptPubKey, dataDir);
            IMempoolValidator validator = context.MempoolValidator;
            Assert.NotNull(validator);

            var destSecret = new BitcoinSecret(new Key(), NetworkContainer.RegTest);
            ...
            tx.Sign(NetworkContainer.RegTest, minerSecret, false);
        ...
        public async Task AcceptToMemoryPool_WithMultiInOutValidTxns_IsSuccessfullAsync()
        {
            ...
            var miner = new BitcoinSecret(new Key(), NetworkContainer.RegTest);
            ITestChainContext context = await TestChainFactory.CreateAsync(NetworkContainer.RegTest, miner.PubKey.Hash.ScriptPubKey, dataDir);
            IMempoolValidator validator = context.MempoolValidator;
            Assert.NotNull(validator);

            var alice = new BitcoinSecret(new Key(), NetworkContainer.RegTest);
            var bob = new BitcoinSecret(new Key(), NetworkContainer.RegTest);
            var satoshi = new BitcoinSecret(new Key(), NetworkContainer.RegTest);
            ...
            var txBuilder = new TransactionBuilder(NetworkContainer.RegTest);
            ...
            txBuilder = new TransactionBuilder(NetworkContainer.RegTest);
        ...
        public async Task AcceptToMemoryPool_WithMultiSigValidTxns_IsSuccessfullAsync()
        {
            ...
            var bob = new BitcoinSecret(new Key(), NetworkContainer.RegTest);
            var satoshi = new BitcoinSecret(new Key(), NetworkContainer.RegTest);
            var nico = new BitcoinSecret(new Key(), NetworkContainer.RegTest);
        ...
        public async Task AcceptToMemoryPool_WithP2SHValidTxns_IsSuccessfullAsync()
        {
            ...
            BitcoinScriptAddress corpRedeemAddress = corpMultiSig.GetScriptAddress(NetworkContainer.RegTest);
            ...
            Coin[] corpCoins = fundP2shTx.Outputs
                        ...
                        .Select(o => ScriptCoin.Create(NetworkContainer.RegTest, new OutPoint(fundP2shTx.GetHash(), fundP2shTx.Outputs.IndexOf(o)), o, corpMultiSig))
                        .ToArray();
        ...
        public async Task AcceptToMemoryPool_WithP2WPKHValidTxns_IsSuccessfullAsync()
        {
            ...
            var miner = new BitcoinSecret(new Key(), NetworkContainer.RegTest);
            ITestChainContext context = await TestChainFactory.CreateAsync(NetworkContainer.RegTest, miner.PubKey.WitHash.ScriptPubKey, dataDir);
            IMempoolValidator validator = context.MempoolValidator;
            Assert.NotNull(validator);
        ...
        public async Task AcceptToMemoryPool_WithP2WSHValidTxns_IsSuccessfullAsync()
        {
            ...
            var miner = new BitcoinSecret(new Key(), NetworkContainer.RegTest);
            ITestChainContext context = await TestChainFactory.CreateAsync(NetworkContainer.RegTest, miner.PubKey.ScriptPubKey.WitHash.ScriptPubKey, dataDir);
            IMempoolValidator validator = context.MempoolValidator;
            Assert.NotNull(validator);
            ...
            // Fund Bob
            // 50 Coins come from first tx on chain - send bob 42 and change back to miner
            ScriptCoin witnessCoin = ScriptCoin.Create(NetworkContainer.RegTest, context.SrcTxs[0].GetHash(), 0, context.SrcTxs[0].TotalOut, miner.PubKey.ScriptPubKey.WitHash.ScriptPubKey, miner.PubKey.ScriptPubKey).AssertCoherent(NetworkContainer.RegTest);
            var txBuilder = new TransactionBuilder(NetworkContainer.RegTest);
        ...
        public async Task AcceptToMemoryPool_WithSegWitValidTxns_IsSuccessfullAsync()
        {
            ...
            var miner = new BitcoinSecret(new Key(), NetworkContainer.RegTest);
            ITestChainContext context = await TestChainFactory.CreateAsync(NetworkContainer.RegTest, miner.PubKey.ScriptPubKey.WitHash.ScriptPubKey.Hash.ScriptPubKey, dataDir);
            IMempoolValidator validator = context.MempoolValidator;
            Assert.NotNull(validator);
            ...
            // Fund Bob
            // 50 Coins come from first tx on chain - send bob 42 and change back to miner
            ScriptCoin witnessCoin =  ScriptCoin.Create(NetworkContainer.RegTest, context.SrcTxs[0].GetHash(), 0, context.SrcTxs[0].TotalOut, miner.PubKey.ScriptPubKey.WitHash.ScriptPubKey.Hash.ScriptPubKey, miner.PubKey.ScriptPubKey);
            var txBuilder = new TransactionBuilder(NetworkContainer.RegTest);
            ...
            Transaction noWitTx = p2shOverp2wpkh.WithOptions(TransactionOptions.None, NetworkContainer.RegTest.Consensus.ConsensusFactory);
        ...
        public async void AcceptToMemoryPool_TxAncestorsConflictSpend_ReturnsFalseAsync()
        {
            ...
            var miner = new BitcoinSecret(new Key(), NetworkContainer.RegTest);
            ITestChainContext context = await TestChainFactory.CreateAsync(NetworkContainer.RegTest, miner.PubKey.Hash.ScriptPubKey, dataDir).ConfigureAwait(false);
            IMempoolValidator validator = context.MempoolValidator;
            var bob = new BitcoinSecret(new Key(), NetworkContainer.RegTest);
            var txBuilder = new TransactionBuilder(NetworkContainer.RegTest);
