...
namespace NBitcoin
{
    ...
    public class TransactionBuilder
    {
        ...
        {
            return BuildTransaction(sign, SigHash.All, out _);
        }
        ...
        /// <param name="sign">True if signs all inputs with the available keys</param>
        /// <param name="sigHash">The type of signature</param>
        /// <param name="coinsSpent">The coins spent.</param>
        /// <returns>The transaction</returns>
        /// <exception cref="NBitcoin.NotEnoughFundsException">Not enough funds are available</exception>
        public Transaction BuildTransaction(bool sign, SigHash sigHash, out List<ICoin> coinsSpent)
        {
            var ctx = new TransactionBuildingContext(this);
            coinsSpent = new List<ICoin>();
            ...
            foreach (BuilderGroup group in this._BuilderGroups)
            {
                ...
                foreach (KeyValuePair<AssetId, List<Builder>> builders in buildersByAsset)
                {
                    ...
                    IEnumerable<ICoin> partialCoinsSpent = BuildTransaction(ctx, group, builders.Value, coins, new AssetMoney(builders.Key));
                    coinsSpent.AddRange(partialCoinsSpent);

                    Money btcSpent = partialCoinsSpent.OfType<IColoredCoin>().Select(c => c.Bearer.Amount).Sum();
                ...
                coinsSpent.AddRange(BuildTransaction(ctx, group, group.Builders, group.Coins.Values.OfType<Coin>().Where(IsEconomical), Money.Zero));
            }
