...
namespace Stratis.Bitcoin.P2P.Peer
{
    ...
    public class NetworkPeerFactory : INetworkPeerFactory
    {
        ...
        /// <returns></returns>
        public NetworkPeer ConnectToLocal(Network network, NetworkPeerConnectionParameters parameters)
        {
            return this.CreateConnectedNetworkPeer(network, Utils.ParseIpEndpoint("localhost", network.DefaultPort), parameters);
        }

        public NetworkPeer ConnectToLocal(Network network, ProtocolVersion myVersion = ProtocolVersion.PROTOCOL_VERSION, bool isRelay = true, CancellationToken cancellation = default(CancellationToken))
        {
            return this.ConnectToLocal(network, new NetworkPeerConnectionParameters()
            {
        ...
        {
            return this.CreateConnectedNetworkPeer(network, Utils.ParseIpEndpoint(endpoint, network.DefaultPort), myVersion, isRelay, cancellation);
        }
        ...
        public NetworkPeer CreateConnectedNetworkPeer(Network network, IPEndPoint endpoint, ProtocolVersion myVersion = ProtocolVersion.PROTOCOL_VERSION, bool isRelay = true, CancellationToken cancellation = default(CancellationToken))
        {
            return this.CreateConnectedNetworkPeer(network, endpoint, new NetworkPeerConnectionParameters()
            {
                ConnectCancellation = cancellation,
                IsRelay = isRelay,
                Version = myVersion,
