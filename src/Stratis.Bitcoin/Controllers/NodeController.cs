    ...
    public class NodeController : Controller
    {
        ...
        public async Task<IActionResult> GetRawTransactionAsync([FromQuery] string trxid, bool verbose = false)
        {
            try
            {
                ...
                if (trx == null)
                {
                    trx = this.blockStore != null ? await this.blockStore.GetTransactionById(txid).ConfigureAwait(false) : null;
                }
        ...
        internal static async Task<ChainedHeader> GetTransactionBlockAsync(uint256 trxid, IFullNode fullNode, ChainIndexer chain)
        {
            ...
            uint256 blockid = blockStore != null ? await blockStore.GetBlockIdByTransactionId(trxid).ConfigureAwait(false) : null;
            if (blockid != null)
            {
