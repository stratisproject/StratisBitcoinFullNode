...
using Stratis.Bitcoin.Features.GeneralPurposeWallet.Helpers;
...
namespace Stratis.Bitcoin.Features.GeneralPurposeWallet.Controllers
{
    ...
    public class GeneralPurposeWalletController : Controller
    {
        private readonly IGeneralPurposeWalletManager walletManager;

        private readonly IGeneralPurposeWalletTransactionHandler walletTransactionHandler;
        ...
        public GeneralPurposeWalletController(
            ILoggerFactory loggerFactory,
            IGeneralPurposeWalletManager walletManager,
            IGeneralPurposeWalletTransactionHandler walletTransactionHandler,
        ...
        public IActionResult Create([FromBody]WalletCreationRequest request)
        {
            ...
            try
            {
                this.walletManager.CreateWallet(request.Password, request.Name);
            ...
            catch (GeneralPurposeWalletException e)
            {
        ...
        public IActionResult Load([FromBody]WalletLoadRequest request)
        {
            ...
            try
            {
                GeneralPurposeWallet wallet = this.walletManager.LoadWallet(request.Password, request.Name);
                return this.Ok();
            }
        ...
        public IActionResult Recover([FromBody]WalletRecoveryRequest request)
        {
            ...
            try
            {
                GeneralPurposeWallet wallet = this.walletManager.RecoverWallet(request.Password, request.Name, request.Mnemonic, request.CreationDate);
		...
		*/
        ...
        public IActionResult GetMaximumSpendableBalance([FromQuery] WalletMaximumBalanceRequest request)
        {
            ...
            try
            {
                var transactionResult = this.walletTransactionHandler.GetMaximumSpendableAmount(new GeneralPurposeWalletAccountReference(request.WalletName, request.AccountName), FeeParser.Parse(request.FeeType), request.AllowUnconfirmed);
        ...
        public IActionResult GetTransactionFeeEstimate([FromQuery]TxFeeEstimateRequest request)
        {
            ...
            try
            {
                ...
                var context = new TransactionBuildContext(
                    new GeneralPurposeWalletAccountReference(request.WalletName, request.AccountName),
        ...
        public IActionResult SendTransaction([FromBody] SendTransactionRequest request)
        {
            ...
            if (!this.connectionManager.ConnectedPeers.Any())
                throw new GeneralPurposeWalletException("Can't send transaction: sending transaction requires at least one connection!");
        ...
        public IActionResult GetUnusedAddress([FromQuery]GetUnusedAddressModel request)
        {
            ...
            try
            {
                var result = this.walletManager.GetUnusedAddress(new GeneralPurposeWalletAccountReference(request.WalletName, request.AccountName));
        ...
        public IActionResult GetUnusedAddresses([FromQuery]GetUnusedAddressesModel request)
        {
            ...
            try
            {
                var result = this.walletManager.GetUnusedAddresses(new GeneralPurposeWalletAccountReference(request.WalletName, request.AccountName), count);
        ...
        public IActionResult GetAllAddresses([FromQuery]GetAllAddressesModel request)
        {
            ...
            try
            {
                GeneralPurposeWallet wallet = this.walletManager.GetWallet(request.WalletName);
                GeneralPurposeAccount account = wallet.GetAccountByCoinType(request.AccountName, this.coinType);
