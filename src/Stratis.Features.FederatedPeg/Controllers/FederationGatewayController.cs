    ...
    public class FederationGatewayController : Controller
    {
        ...
        public IActionResult GetMaturedBlockDeposits([FromBody] MaturedBlockRequestModel blockRequest)
        {
            ...
            if (!this.ModelState.IsValid)
            {
                ...
                return ErrorHelpers.BuildErrorResponse(HttpStatusCode.OK, "Formatting error", string.Join(Environment.NewLine, errors));
            }
            ...
            try
            {
                ...
                this.logger.LogTrace("{0}:{1},{2}:{3},{4}:{5}", nameof(depositsResult.ErrorMessage), depositsResult.ErrorMessage, nameof(depositsResult.Succeeded), depositsResult.Succeeded, nameof(depositsResult.Value), depositsResult.Value);
