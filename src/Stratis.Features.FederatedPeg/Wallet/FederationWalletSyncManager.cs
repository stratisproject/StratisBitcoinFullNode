...
namespace Stratis.Features.FederatedPeg.Wallet
{
    public class FederationWalletSyncManager : IFederationWalletSyncManager, IDisposable
    {
        ...
        public void Initialize()
        {
            ...
            this.walletTip = this.chain.Get(this.walletManager.WalletTipHash);
        ...
        private async Task OnProcessBlockAsync(Block block, CancellationToken cancellationToken)
        {
            ...
            ChainedHeader newTip = this.chain.Get(block.GetHash());
            if (newTip == null)
            {
            ...
            if (block.Header.HashPrevBlock != this.walletTip.HashBlock)
            {
                ...
                ChainedHeader inBestChain = this.chain.Get(this.walletTip.HashBlock);
                if (inBestChain == null)
                {
                    ...
                    while (this.chain.Get(fork.HashBlock) == null)
                        fork = fork.Previous;
        ...
        {
            ChainedHeader chainedBlock = this.chain.Get(height);
