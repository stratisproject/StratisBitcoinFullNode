...
namespace Stratis.Features.FederatedPeg.TargetChain
{
    public class CrossChainTransferStore : ICrossChainTransferStore
    {
            ...
            IFederationWalletManager federationWalletManager, IWithdrawalTransactionBuilder withdrawalTransactionBuilder, DBreezeSerializer dBreezeSerializer)
        {
            ...
            this.TipHashAndHeight = this.chainIndexer.Get(0);
        ...
        private void Put(List<Block> blocks)
        {
            ...
            if (allDepositIds.Count == 0)
            {
                ...
                this.TipHashAndHeight = this.chainIndexer.Get(blocks.Last().GetHash());
            ...
            using (DBreeze.Transactions.Transaction dbreezeTransaction = this.DBreeze.GetTransaction())
            {
                ...
                try
                {
                    ...
                    // Commit additions
                    ChainedHeader newTip = this.chainIndexer.Get(blocks.Last().GetHash());
        ...
        private bool RewindIfRequired()
        {
            ...
            if (this.chainIndexer.Get(tipToChase.Hash) == null)
            {
            ...
            if (this.TipHashAndHeight != null && (this.TipHashAndHeight.Height > tipToChase.Height ||
                this.chainIndexer.Get(this.TipHashAndHeight.HashBlock)?.Height != this.TipHashAndHeight.Height))
            {
                // We are ahead of the current chain or on the wrong chain.
                ChainedHeader fork = this.chainIndexer.FindFork(this.TipHashAndHeight.GetLocator()) ?? this.chainIndexer.Get(0);
        ...
        private bool SynchronizeBatch()
        {
            ...
            {
                if (this.chainIndexer.Get(header.HashBlock) == null)
                    break;
        ...
        private ChainedHeader LoadTipHashAndHeight(DBreeze.Transactions.Transaction dbreezeTransaction)
        {
            ...
            this.TipHashAndHeight = this.chainIndexer.Get(blockLocator.Blocks[0]) ?? this.chainIndexer.FindFork(blockLocator);
