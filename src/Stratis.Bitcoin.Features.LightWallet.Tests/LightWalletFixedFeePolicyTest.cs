...
namespace Stratis.Bitcoin.Features.LightWallet.Tests
{
    ...
    public class LightWalletFixedFeePolicyTest
    {
        ...
        {
            ILoggerFactory loggerFactory = new LoggerFactory();
            NodeSettings nodeSettings = new NodeSettings(Network.StratisTest).LoadArguments(new string[0]).LoadConfiguration();
        ...
        {
            ILoggerFactory loggerFactory = new LoggerFactory();
            NodeSettings nodeSettings = new NodeSettings(Network.TestNet).LoadArguments(new string[0]).LoadConfiguration();
            LightWalletFixedFeePolicy policy = new LightWalletFixedFeePolicy(loggerFactory, nodeSettings);
