    ...
    public class MinerTests
    {
        ...
        public class TestContext
        {
            ...
            public async Task InitializeAsync()
            {
                ...
                await this.consensus.ValidateAndExecuteBlockAsync(new RuleContext(new ValidationContext { Block = this.newBlock.Block }, this.network.Consensus, this.consensus.Tip) { CheckPow = false, CheckMerkleRoot = false });
                ...
                for (int i = 0; i < this.blockinfo.Count; ++i)
                {
                    ...
                    await this.consensus.ValidateAndExecuteBlockAsync(new RuleContext(new ValidationContext { Block = pblock }, this.network.Consensus, this.consensus.Tip) { CheckPow = false, CheckMerkleRoot = false });
                    blocks.Add(pblock);
                }
