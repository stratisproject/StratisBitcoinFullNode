    ...
    public partial class RetrieveFromBlockStoreSpecification : BddSpecification
    {
        ...
        private void trying_to_retrieve_the_blocks_from_the_blockstore()
        {
            this.retrievedBlocks = this.blockIds.Concat(new[] { this.wrongBlockId })
                .Select(id => this.node.FullNode.BlockStore().GetBlock(id).GetAwaiter().GetResult()).Select(b => b).ToList();
        ...
        {
            this.retrievedTransaction = this.node.FullNode.BlockStore().GetTransactionById(this.transaction.GetHash()).GetAwaiter().GetResult();
            this.wontRetrieveTransaction = this.node.FullNode.BlockStore().GetTransactionById(this.wrongTransactionId).GetAwaiter().GetResult();
        }
        ...
        private void trying_to_retrieve_the_block_containing_the_transactions_from_the_blockstore()
        {
            this.retrievedBlockId = this.node.FullNode.BlockStore()
                .GetBlockIdByTransactionId(this.transaction.GetHash()).GetAwaiter().GetResult();
            this.wontRetrieveBlockId = this.node.FullNode.BlockStore()
                .GetTransactionById(this.wrongTransactionId).GetAwaiter().GetResult();
        }
