...
namespace Stratis.Bitcoin.IntegrationTests.Wallet
{
    public partial class SendingToAndFromManyAddressesSpecification : BddSpecification
    {
        ...
        {
            // Node1 balance : 150 [150 spendable]
            TestHelper.MineBlocks(this.firstNode, 3);
            this.firstNode.WalletBalance(WalletName).Should().Be(Money.Coins(150));
            this.firstNode.WalletSpendableTransactionCount(WalletName).Should().Be(3);
            ...
            // This will create a transaction with an output of a 100 coins as min confirmations is set to 2:
            this.transactionBuildContext = TestHelper.CreateTransactionBuildContext(this.firstNode.FullNode.Network, WalletName, WalletAccountName, WalletPassword, nodeTwoRecipients, FeeType.Medium, 2);
            Transaction transaction = this.firstNode.FullNode.WalletTransactionHandler().BuildTransaction(this.transactionBuildContext);

            // As only 50 coins will be sent from the 100 selected, we need to test the change amount.
            this.firstNodeTransactionFee = this.firstNode.GetFee(this.transactionBuildContext);
            this.firstNodeChangeAmount = transaction.TotalOut - Money.Coins(50);
            this.firstNodeChangeAmount.Should().Be(Money.Coins(100) - Money.Coins(50) - this.firstNodeTransactionFee);
            transaction.TotalOut.Should().Be(Money.Coins(100) - this.firstNodeTransactionFee);
        ...
        private void node2_receives_the_funds()
        {
            ...
            this.firstNode.WalletBalance(WalletName).Should().Be(Money.Coins(50) + this.firstNodeChangeAmount);
            this.firstNode.WalletHeight(WalletName).Should().Be(3);
            ...
            this.secondNode.WalletBalance(WalletName).Should().Be(Money.Coins(50));
            ...
            // Node2 balance : 100
            this.secondNode.WalletBalance(WalletName).Should().Be(Money.Coins(100) + this.firstNodeTransactionFee);
            this.secondNode.WalletHeight(WalletName).Should().Be(4);
        }
        ...
        private void node1_receives_the_funds()
        {
            ...
            nodeOneBeforeBalance.Should().Be(Money.Coins(100) - this.firstNodeTransactionFee);
            TestHelper.MineBlocks(this.secondNode, 1);
