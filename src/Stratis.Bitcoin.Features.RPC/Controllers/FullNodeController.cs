...
namespace Stratis.Bitcoin.Features.RPC.Controllers
{
    ...
    public class FullNodeController : FeatureController
    {
        ...
        public object GetBlockHeader(string hash, bool isJsonFormat = true)
        {
            ...
            {
                BlockHeader blockHeader = this.ChainIndexer.Get(uint256.Parse(hash))?.Header;
                if (blockHeader != null)
                {
                    if (isJsonFormat)
                    {
        ...
        public async Task<object> GetBlockAsync(string blockHash, int verbosity = 0)
        {
            ...
            return new BlockModel(block, this.ChainIndexer.Get(block.GetHash()), this.ChainIndexer.Tip, this.Network, verbosity);
        }
        ...
        private async Task<ChainedHeader> GetTransactionBlockAsync(uint256 trxid)
        {
            ...
            if (blockid != null)
                block = this.ChainIndexer?.Get(blockid);
