...
namespace Stratis.Bitcoin.Features.BlockStore.Tests
{
    public sealed class PruneBlockRepositoryTests : LogsTestBase
    {
        ...
        public async Task PruneRepository_PruneAndCompact_FromGenesis_OnStartUpAsync()
        {
            ...
            await blockRepository.PutBlocks(new HashHeightPair(posBlocks.Last().GetHash(), 50), posBlocks);
            ...
            Assert.Null(await blockRepository.GetBlock(chainedHeaderTip.GetAncestor(39).HashBlock));
        }
        ...
        public async Task PruneRepository_PruneAndCompact_MidChain_OnStartUpAsync()
        {
            ...
            var blockRepository = new BlockRepository(this.Network, dataFolder, this.LoggerFactory.Object, dBreezeSerializer);
            await blockRepository.PutBlocks(new HashHeightPair(posBlocks.Take(100).Last().GetHash(), 100), posBlocks.Take(100).ToList());
            ...
            // Ensure that the block has been deleted from disk.
            Assert.Null(await blockRepository.GetBlock(chainedHeaderTip.GetAncestor(49).HashBlock));

            // Push more blocks to the repository and prune again.
            // This will delete blocks from height 150 to 50.
            await blockRepository.PutBlocks(new HashHeightPair(posBlocks.Skip(100).Take(100).Last().GetHash(), 200), posBlocks.Skip(100).Take(100).ToList());
            ...
            // Ensure that the block has been deleted from disk.
            Assert.Null(await blockRepository.GetBlock(chainedHeaderTip.GetAncestor(149).HashBlock));
        }
        ...
        public async Task PruneRepository_PruneAndCompact_OnShutDownAsync()
        {
            ...
            await blockRepository.DeleteBlocks(posBlocks.Take(30).Select(b => b.GetHash()).ToList());
            prunedBlockRepository.UpdatePrunedTip(chainedHeaderTip.GetAncestor(30));
            // Ensure that the block has been deleted from disk.
            Assert.Null(await blockRepository.GetBlock(chainedHeaderTip.GetAncestor(29).HashBlock));
