        ...
        public async Task<IActionResult> GetBlockAsync([FromQuery] SearchByHashRequest query)
        {
            ...
            try
            {
                ...
                return query.ShowTransactionDetails
                    ? this.Json(new BlockTransactionDetailsModel(block, this.chainIndexer.Get(block.GetHash()), this.chainIndexer.Tip, this.network))
                    : this.Json(new BlockModel(block, this.chainIndexer.Get(block.GetHash()), this.chainIndexer.Tip, this.network));
            }
            catch (Exception e)
            {
