        ...
        public IActionResult GetWatchOnlyWallet()
        {
            try
            {
                ...
                foreach (KeyValuePair<string, WatchedAddress> watchAddress in watchOnlyWallet.WatchedAddresses)
                {
                    ...
                    {
                        Transaction transaction = watchOnlyWallet.Network.Consensus.ConsensusFactory.CreateTransaction(transactionData.Value.Hex);
                ...
                foreach (KeyValuePair<string, TransactionData> transaction in watchOnlyWallet.WatchedTransactions)
                {
                    ...
                    {
                        Transaction = new TransactionVerboseModel(watchOnlyWallet.Network.Consensus.ConsensusFactory.CreateTransaction(transaction.Value.Hex), watchOnlyWallet.Network)
                    };
