...
namespace Stratis.Bitcoin.Features.Consensus.Behaviors
{
   public class ProvenHeadersReservedSlotsBehavior : NetworkPeerBehavior
   {
      ...
      {
         bool isBelowLastCheckpoint = this.consensusSettings.UseCheckpoints && (this.chainState.ConsensusTip.Height <= this.checkpoints.GetLastCheckpointHeight());

         // We enforce having free slots for PH enabled nodes only when we have passed last checkpoint.
         if (!isBelowLastCheckpoint)
         {
