...
namespace Stratis.Bitcoin.Features.Consensus.Rules.CommonRules
{
    ...
    public class BlockSizeRule : ConsensusRule
    {
        ...
        public override Task RunAsync(RuleContext context)
        {
            ...
            // failed).
            if (this.GetBlockWeight(context.BlockValidationContext.Block, options, TransactionOptions.All) > options.MaxBlockWeight)
            {
            ...
            if ((block.Transactions.Count == 0) || (block.Transactions.Count > options.MaxBlockBaseSize) || 
                (GetSize(this.Parent.Network, block, TransactionOptions.All & ~TransactionOptions.All) > options.MaxBlockBaseSize))
            {
        ...
        public long GetBlockWeight(Block block, PowConsensusOptions powOptions, TransactionOptions options)
        {
            return GetSize(this.Parent.Network, block, options & ~TransactionOptions.Witness) * (powOptions.WitnessScaleFactor - 1) + GetSize(this.Parent.Network, block, options);
        }
