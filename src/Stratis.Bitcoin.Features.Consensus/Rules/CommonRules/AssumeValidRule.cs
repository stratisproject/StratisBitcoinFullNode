    ...
    public class AssumeValidRule : ConsensusRule
    {
        ...
        public override Task RunAsync(RuleContext context)
        {
            ...
            if (!context.SkipValidation && (this.Parent.ConsensusSettings.BlockAssumedValid != null))
            {
                ...
                context.SkipValidation = (assumeValidBlock != null) && (context.ValidationContext.ChainTipToExtand.Height <= assumeValidBlock.Height);
                if (context.SkipValidation)
                    this.Logger.LogTrace("Block validation will be partially skipped due to block height {0} is not greater than assumed valid block height {1}.", context.ValidationContext.ChainTipToExtand.Height, assumeValidBlock.Height);
            }
