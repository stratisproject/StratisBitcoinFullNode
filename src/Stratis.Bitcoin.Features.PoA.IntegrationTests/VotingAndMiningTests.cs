...
namespace Stratis.Bitcoin.Features.PoA.IntegrationTests
{
    public class VotingAndMiningTests : IDisposable
    {
        ...
        public async Task CantVoteTwiceAsync()
        {
            ...
            this.node1.FullNode.NodeService<DefaultVotingController>().VoteAddFedMember(model);
            ...
            this.node2.FullNode.NodeService<DefaultVotingController>().VoteAddFedMember(model);
        ...
        public async Task CanSyncIfFedMemberKickedAsync()
        {
            ...
            this.node1.FullNode.NodeService<DefaultVotingController>().VoteKickFedMember(model);
            this.node2.FullNode.NodeService<DefaultVotingController>().VoteKickFedMember(model);
        ...
        private async Task VoteAndMineBlockAsync(PubKey key, bool add, CoreNode node)
        {
            ...
            if (add)
                node.FullNode.NodeService<DefaultVotingController>().VoteAddFedMember(model);
            else
                node.FullNode.NodeService<DefaultVotingController>().VoteKickFedMember(model);
