...
namespace Stratis.Bitcoin.Features.Consensus.Tests
{
    public class PeerBanningTest
    {
        ...
        public async Task NodeIsSynced_PeerSendsABadBlockAndPeerIsConnected_ThePeerGetsBanned_Async()
        {
            ...
            using (var peer = new NetworkPeer(context.DateTimeProvider, context.LoggerFactory))
            {
                peer.Behaviors.Add(connectionManagerBehavior);
                context.MockReadOnlyNodesCollection.Setup(s => s.FindByEndpoint(It.IsAny<IPEndPoint>())).Returns(peer);
