    ...
    public class PosCoinViewRuleTests : TestPosConsensusRulesUnitTestBase
    {
        ...
        private async Task<ConsensusManager> CreateConsensusManagerAsync(Dictionary<uint256, UnspentOutputs> unspentOutputs)
        {
            ...
            ConsensusRuleEngine consensusRuleEngine = new PosConsensusRuleEngine(this.network, this.loggerFactory.Object, DateTimeProvider.Default,
                this.ConsensusChainIndexer, this.nodeDeployments, this.consensusSettings, this.checkpoints.Object, this.coinView.Object, this.stakeChain.Object,
            ...
            var consensus = new ConsensusManager(chainedHeaderTree, this.network, this.loggerFactory.Object, this.chainState.Object, integrityValidator,
                ...
                new Mock<IPeerBanning>().Object, initialBlockDownloadState, this.ConsensusChainIndexer, new Mock<IBlockPuller>().Object, new Mock<IBlockStore>().Object,
            ...
            this.coinView.Setup(d => d.FetchCoins(It.IsAny<uint256[]>(), It.IsAny<CancellationToken>()))
                .Returns((uint256[] txIds, CancellationToken cancel) =>
                {
                    ...
                    return new FetchCoinsResponse(result, this.ConsensusChainIndexer.Tip.HashBlock);
                });

            // Mock the coinviews "GetTipHashAsync" method.
            this.coinView.Setup(d => d.GetTipHash(It.IsAny<CancellationToken>())).Returns(() =>
            {
                return this.ConsensusChainIndexer.Tip.HashBlock;
            });
            ...
            this.stakeChain.Setup(d => d.Get(It.IsAny<uint256>())).Returns(new BlockStake()
            {
                ...
                StakeModifierV2 = 0,
                StakeTime = (this.ConsensusChainIndexer.Tip.Header.Time + 60) & ~PosConsensusOptions.StakeTimestampMask
            });

            // Since we are mocking the chainState ensure that the BlockStoreTip returns a usable value.
            this.chainState.Setup(d => d.BlockStoreTip).Returns(this.ConsensusChainIndexer.Tip);

            // Since we are mocking the chainState ensure that the ConsensusTip returns a usable value.
            this.chainState.Setup(d => d.ConsensusTip).Returns(this.ConsensusChainIndexer.Tip);

            // Initialize the consensus manager.
            await consensus.InitializeAsync(this.ConsensusChainIndexer.Tip);
        ...
        public async Task PosCoinViewRuleFailsAsync()
        {
            ...
            transaction.Inputs.Add(TxIn.CreateCoinbase(this.ConsensusChainIndexer.Tip.Height + 1));
            ...
            uint blockTime = (this.ConsensusChainIndexer.Tip.Header.Time + 60) & ~PosConsensusOptions.StakeTimestampMask;
            ...
            block.Header.HashPrevBlock = this.ConsensusChainIndexer.Tip.HashBlock;
            block.Header.Time = blockTime;
            block.Header.Bits = block.Header.GetWorkRequired(this.network, this.ConsensusChainIndexer.Tip);
