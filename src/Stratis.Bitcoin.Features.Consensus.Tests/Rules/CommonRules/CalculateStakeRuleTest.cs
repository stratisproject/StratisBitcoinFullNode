...
namespace Stratis.Bitcoin.Features.Consensus.Tests.Rules.CommonRules
{
    public class CalculateStakeRuleTest : TestPosConsensusRulesUnitTestBase
    {
        ...
        {
            this.ConsensusChainIndexer = GenerateChainWithHeight(5, this.network);
        ...
        public async Task RunAsync_ProofOfStakeBlock_SetsStake_SetsNextWorkRequiredAsync()
        {
            ...
            block.AddTransaction(CreateCoinStakeTransaction(this.network, new Key(), 6, this.ConsensusChainIndexer.GetBlock(5).HashBlock));
            ...
            this.stakeValidator.Setup(s => s.GetNextTargetRequired(
                ...
                this.ConsensusChainIndexer.GetBlock(3),
            ...
            Assert.Equal(this.ConsensusChainIndexer.GetBlock(5).HashBlock, (this.ruleContext as PosRuleContext).BlockStake.PrevoutStake.Hash);
        }
        ...
        public async Task RunAsync_ProofOfWorkBlock_CheckPow_ValidPow_SetsStake_SetsNextWorkRequiredAsync()
        {
            ...
            this.ConsensusChainIndexer = MineChainWithHeight(2, this.network);
            this.consensusRules = this.InitializeConsensusRules();

            this.ruleContext.ValidationContext = new ValidationContext()
            {
                BlockToValidate = TestRulesContextFactory.MineBlock(this.network, this.ConsensusChainIndexer),
                ChainedHeaderToValidate = this.ConsensusChainIndexer.Tip
            };
            ...
            this.stakeValidator.Setup(s => s.GetNextTargetRequired(
                this.stakeChain.Object,
                this.ConsensusChainIndexer.GetBlock(1),
                this.network.Consensus,
                false))
                .Returns(target)
                .Verifiable();
