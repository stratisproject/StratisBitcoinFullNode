...
namespace Stratis.Bitcoin.Features.Consensus.Tests.Rules.CommonRules
{
    public class ConsensusRulesRegistrationTest
    {
        ...
        {
            foreach (Network network in new[] { new BitcoinMain(), new BitcoinTest(), new BitcoinRegTest() })
            {
        ...
        {
            foreach (Network network in new[] { new StratisMain(), new StratisTest(), new StratisRegTest() })
            {
                List<Type> headerValidationRules = network.Consensus.ConsensusRules.HeaderValidationRules;
                ...
                integrityValidationRules[0].FullName.Should().Be(typeof(PosTransactionsOrderRule).FullName);
