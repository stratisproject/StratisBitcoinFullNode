...
namespace Stratis.Bitcoin.Features.Wallet.Tests
{
    public class WalletSyncManagerTest : LogsTestBase
    {
        ...
        public void Start_BlockNotChain_ReorgsWalletManagerUsingWallet()
        {
            ...
            ChainedHeader forkBlock = this.chainIndexer.Get(3); // use a block as the fork to recover to.
        ...
        public void ProcessBlock_NewBlock_PreviousHashSameAsWalletTip_PassesBlockToManagerWithoutReorg()
        {
            ...
            var walletSyncManager = new WalletSyncManagerOverride(this.LoggerFactory.Object, this.walletManager.Object, this.chainIndexer, KnownNetworks.StratisMain,
                this.blockStore.Object, this.storeSettings, this.signals);
            walletSyncManager.SetWalletTip(this.chainIndexer.Get(3));
        ...
        public void ProcessBlock_NewBlock_BlockNotOnBestChain_ReOrgWalletManagerUsingBlockStoreCache()
        {
            ...
            walletSyncManager.SetWalletTip(leftChainIndexer.Get(result.LeftForkBlocks[3].Header.GetHash()));
            ...
            this.walletManager.Verify(w => w.RemoveBlocks(ExpectChainedBlock(this.chainIndexer.Get(2))));

            //verify manager processes each missing block until caught up.
            // height 3
            this.walletManager.Verify(w => w.ProcessBlock(ExpectBlock(result.RightForkBlocks[2]), ExpectChainedBlock(this.chainIndexer.Get(3))));
            // height 4
            this.walletManager.Verify(w => w.ProcessBlock(ExpectBlock(result.RightForkBlocks[3]), ExpectChainedBlock(this.chainIndexer.Get(4))));
            // height 5
            this.walletManager.Verify(w => w.ProcessBlock(ExpectBlock(result.RightForkBlocks[4]), ExpectChainedBlock(this.chainIndexer.Get(5))), Times.Exactly(2));
        }
        ...
        public void ProcessBlock_NewBlock__BlockOnBestChain_ReOrgWalletManagerUsingBlockStoreCache()
        {
            ...
            walletSyncManager.SetWalletTip(this.chainIndexer.Get(2));
            ...
            //verify manager processes each missing block until caught up.
            // height 3
            this.walletManager.Verify(w => w.ProcessBlock(ExpectBlock(blocks[2]), ExpectChainedBlock(this.chainIndexer.Get(3))));
            // height 4
            this.walletManager.Verify(w => w.ProcessBlock(ExpectBlock(blocks[3]), ExpectChainedBlock(this.chainIndexer.Get(4))), Times.Exactly(2));
        }
        ...
        public void SyncFromDate_GivenDateMatchingBlocksOnChain_UpdatesUsingClosestBlock()
        {
            ...
            walletSyncManager.SyncFromDate(this.chainIndexer.Get(3).Header.BlockTime.DateTime.AddDays(2));

            uint256 expectedHash = this.chainIndexer.Get(3).HashBlock;
        ...
        public void SyncFromDate_GivenDateNotMatchingBlocksOnChain_UpdatesUsingFirstBlock()
        {
            ...
            uint256 expectedHash = this.chainIndexer.Get(1).HashBlock;
            Assert.Equal(walletSyncManager.WalletTip.HashBlock, expectedHash);
            this.walletManager.VerifySet(w => w.WalletTipHash = expectedHash);
        }
        ...
        public void SyncFromHeight_BlockWithHeightOnChain_UpdatesWalletTipOnWalletAndWalletSyncManagers()
        {
            ...
            uint256 expectedHash = this.chainIndexer.Get(2).HashBlock;
            Assert.Equal(walletSyncManager.WalletTip.HashBlock, expectedHash);
            this.walletManager.VerifySet(w => w.WalletTipHash = expectedHash);
        }
        ...
        public void ProcessBlock_With_No_Wallet_Processing_Is_Ignored()
        {
            ...
            walletSyncManager.SetWalletTip(this.chainIndexer.Get(1));
        ...
        {
            uint256 expectedBlockHash = this.chainIndexer.Get(blockHeight).Header.GetHash();
