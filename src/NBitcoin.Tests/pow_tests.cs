...
namespace NBitcoin.Tests
{
    public class pow_tests
    {
        ...
        {
            var chain = new ConcurrentChain(NetworkContainer.Main);
            ...
            {
                Target thisWork = block.GetWorkRequired(NetworkContainer.Main);
                Target thisWork2 = block.Previous.GetNextWorkRequired(NetworkContainer.Main);
                Assert.Equal(thisWork, thisWork2);
                Assert.True(block.CheckProofOfWorkAndTarget(NetworkContainer.Main));
            }
        }
