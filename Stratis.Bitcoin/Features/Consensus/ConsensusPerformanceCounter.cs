	...
	public class ConsensusPerformanceSnapshot
	{
		...
		public override string ToString()
		{
			...
			if(this.TotalProcessedInputs > 0)
				builder.AppendLine("Inputs:".PadRight(LogsExtension.ColumnLength) + (this.TotalBlockFetchingTime.TotalMilliseconds / this.TotalProcessedInputs).ToString("0.0000") + " ms/input");
			if(this.TotalProcessedTransactions > 0)
				builder.AppendLine("Transactions:".PadRight(LogsExtension.ColumnLength) + (this.TotalBlockFetchingTime.TotalMilliseconds / this.TotalProcessedTransactions).ToString("0.0000") + " ms/tx");
			if(this.TotalProcessedBlocks > 0)
				builder.AppendLine("Blocks:".PadRight(LogsExtension.ColumnLength) + (this.TotalBlockFetchingTime.TotalMilliseconds / this.TotalProcessedBlocks).ToString("0.0000") + " ms/block");
			builder.AppendLine("====Validation Speed====");
			if(this.TotalProcessedInputs > 0)
				builder.AppendLine("Inputs:".PadRight(LogsExtension.ColumnLength) + (this.TotalBlockValidationTime.TotalMilliseconds / this.TotalProcessedInputs).ToString("0.0000") + " ms/inputs");
			if(this.TotalProcessedTransactions > 0)
				builder.AppendLine("Transactions:".PadRight(LogsExtension.ColumnLength) + (this.TotalBlockValidationTime.TotalMilliseconds / this.TotalProcessedTransactions).ToString("0.0000") + " ms/tx");
			if(this.TotalProcessedBlocks > 0)
				builder.AppendLine("Blocks:".PadRight(LogsExtension.ColumnLength) + (this.TotalBlockValidationTime.TotalMilliseconds / this.TotalProcessedBlocks).ToString("0.0000") + " ms/tx");
			builder.AppendLine("====UTXO Fetching Speed====");
			if(this.TotalProcessedInputs > 0)
				builder.AppendLine("Inputs:".PadRight(LogsExtension.ColumnLength) + (this.TotalUTXOFetchingTime.TotalMilliseconds / this.TotalProcessedInputs).ToString("0.0000") + " ms/inputs");
			if(this.TotalProcessedTransactions > 0)
				builder.AppendLine("Transactions:".PadRight(LogsExtension.ColumnLength) + (this.TotalUTXOFetchingTime.TotalMilliseconds / this.TotalProcessedTransactions).ToString("0.0000") + " ms/tx");
			if(this.TotalProcessedBlocks > 0)
				builder.AppendLine("Blocks:".PadRight(LogsExtension.ColumnLength) + (this.TotalUTXOFetchingTime.TotalMilliseconds / this.TotalProcessedBlocks).ToString("0.0000") + " ms/tx");
			...
			if(total > 0)
			{
				builder.AppendLine("Blk Fetching:".PadRight(LogsExtension.ColumnLength) + ((decimal)this._TotalBlockFetchingTime * 100m / total).ToString("0.00") + " %");
				builder.AppendLine("Validation:".PadRight(LogsExtension.ColumnLength) + ((decimal)this._TotalBlockValidationTime * 100m / total).ToString("0.00") + " %");
				builder.AppendLine("Utxo Fetching:".PadRight(LogsExtension.ColumnLength) + ((decimal)this._TotalUTXOFetchingTime * 100m / total).ToString("0.00") + " %");
			}
			builder.AppendLine("==========================");
