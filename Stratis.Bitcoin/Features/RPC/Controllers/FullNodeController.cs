...
namespace Stratis.Bitcoin.Features.RPC.Controllers
{
	public class FullNodeController : BaseRPCController
	{
		...
		/// <summary>
		/// Implements getblock RPC call.
		/// </summary>
		/// <param name="hash">Hash of block.</param>
		/// <param name="isJsonFormat">Indicates whether to provide data in Json or binary format.</param>
		/// <returns>The block header rpc format.</returns>
		[ActionName("getblock")]
		public BlockModel GetBlock(string hash, bool isJsonFormat = true)
		{
			Guard.NotNull(hash, nameof(hash));

			this.logger.LogDebug("RPC GetBlock {0}", hash);

			if (!isJsonFormat)
			{
				this.logger.LogError("Binary serialization is not supported for RPC {0}", nameof(GetBlock));
				throw new NotImplementedException();
			}
