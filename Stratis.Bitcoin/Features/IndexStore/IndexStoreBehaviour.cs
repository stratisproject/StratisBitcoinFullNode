...
{
    public interface IIndexStoreBehavior : INodeBehavior
    {
    ...
    public class IndexStoreBehavior : NodeBehavior
    {
        ...
        private readonly IIndexStoreCache indexStoreCache;
        ...
        public IndexStoreBehavior(ConcurrentChain chain, IndexRepository indexRepository, IndexStoreCache indexStoreCache, ILogger logger)
            : this(chain, indexRepository as IIndexRepository, indexStoreCache as IIndexStoreCache, logger)
        {
        }

        public IndexStoreBehavior(ConcurrentChain chain, IIndexRepository indexRepository, IIndexStoreCache indexStoreCache, ILogger logger)
        {
            Guard.NotNull(chain, nameof(chain));
            Guard.NotNull(indexRepository, nameof(indexRepository));
            Guard.NotNull(indexStoreCache, nameof(indexStoreCache));
            Guard.NotNull(indexStoreCache, nameof(logger));
        ...
        private async Task ProcessGetDataAsync(Node node, GetDataPayload getDataPayload)
        {
            ...
            foreach (var item in getDataPayload.Inventory.Where(inv => inv.Type.HasFlag(InventoryType.MSG_BLOCK)))
            {
                ...
                var block = await this.indexStoreCache.GetBlockAsync(item.Hash).ConfigureAwait(false);
        ...
        {
            return new IndexStoreBehavior(this.chain, this.indexRepository, this.indexStoreCache, this.logger)
            {
