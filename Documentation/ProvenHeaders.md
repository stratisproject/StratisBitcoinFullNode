...
This means that if we are synced till the block of height `x` and we receive a proven header that represents a block within the range from `x + 1` to `x + coinstakeAge` then we have enough data to validate the coinstake's kernel because a valid kernel couldn't have been originated in a block with height greater than `coinstakeAge`. 
...
This document is written under assumption that a softfork that changes `coinstakeAge` consensus parameter from 50 to 500 is activated for Stratis.
